...
{
    public partial class FormZippedHexahedronGridderElement : Form
    {
        public FormZippedHexahedronGridderElement()
        {
            InitializeComponent();
        ...
        private void Create3DObject(object sender, EventArgs e)
        {
            try
            {
                ...
                ZippedCatesianGridderSource source = new ZippedCatesianGridderSource() { NX = nx, NY = ny, NZ = nz, DX = dxArray, DY = dyArray, DZ = dzArray, };
                ...
                // 用线性渐变的颜色便于调试。
                ZippedHexahedronGridderHelper.LinearValue(source.DimenSize, minValue, maxValue, out gridIndexes, out gridValues);
                ...
                ZippedMeshGeometry mesh = ZippedHexahedronGridderHelper.CreateMesh(source);//.CreateMesh(source);
                DateTime t1 = DateTime.Now;
                TimeSpan ts1 = t1 - t0;

                mesh.VertexColors = ZippedHexahedronGridderHelper.FromColors(source, gridIndexes, colors, mesh.Visibles);
                //this.DebugMesh(mesh);

                ZippedHexahedronGridderElement gridderElement = new ZippedHexahedronGridderElement(source, this.scientificVisual3DControl.Scene.CurrentCamera);
